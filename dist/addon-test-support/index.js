...
var DOMAssertions = /** @class */ (function () {
    function DOMAssertions(target, rootElement, testContext, targetDescription) {
        if (targetDescription === void 0) { targetDescription = elementToString(target); }
        this.target = target;
     ...
     */
    DOMAssertions.prototype.containingText = function (expected) {
        var predicate;
        if (typeof expected === 'string') {
            var collapsedExpected_1 = collapseWhitespace(expected);
            predicate = function (element) { return collapseWhitespace(element.textContent) === collapsedExpected_1; };
        }
        else if (expected instanceof RegExp) {
            predicate = function (element) { return expected.test(element.textContent); };
        }
        else {
            throw new Error("You must pass a string or Regular Expression to \"containingText\". You passed " + expected);
        }
        var target = this.findElements().find(predicate);
        return new DOMAssertions(target || null, this.rootElement, this.testContext, this.targetDescription + " containing text " + expected);
    };
    /**
     * @private
     */
